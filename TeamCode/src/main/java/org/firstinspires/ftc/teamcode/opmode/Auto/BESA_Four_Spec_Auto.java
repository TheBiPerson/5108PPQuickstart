...
public class BESA_Four_Spec_Auto extends OpMode {
...
//********************* Begin Pedro Pathing Pose and Chains **********************************
    ...
    private PathChain preload, twoSamplePush, /*intake,*/ score2, return2, score3, return3, score4, park; // clearer name inline with poses
    //intake is unnecessary as it can be the end of pathchain twoSamplePush with no consequences
    public int pathState = 0;
    //set poses
    private Pose startingPose = new Pose(8.5,66, Math.toRadians(0));

    private Pose firstOnBar = new Pose(25,66,Math.toRadians(0)); // preload
    private Pose secondOnBar = new Pose(29.5,64,Math.toRadians(0)); // wallPickup1
    private Pose thirdOnBar = new Pose(28.5,68,Math.toRadians(0)); // wallPickup2
    private Pose fourthOnBar = new Pose(28,72,Math.toRadians(0)); // wallPickup3
    private Pose fifthOnBar = new Pose(28,74,Math.toRadians(0)); // wallPickup4

    private Pose wallPickup1 = new Pose(23,28,Math.toRadians(180)); // secondOnBar
    private Pose wallPickup2 = new Pose(22,38,Math.toRadians(180)); // thirdOnBar
    private Pose wallPickup3 = new Pose(22,38,Math.toRadians(180)); // fourthOnBar
    private Pose wallPickup4 = new Pose(24,28,Math.toRadians(180)); // fifthOnBar
    ...
    private void buildPaths() {
        ...
        twoSamplePush = follower.pathBuilder() // pushes 2 samples back and readies to pickup specimen 2
                ...
                ).setLinearHeadingInterpolation(Math.toRadians(-45),firstSampleLineup.getHeading())
                //push first sample
                .addPath(new BezierLine(
                        new Point(firstSampleLineup),
                ...
                ).setLinearHeadingInterpolation(firstSampleBack.getHeading(),secondSampleLineup.getHeading())
                // push second sample
                .addPath(new BezierLine(
                        new Point(secondSampleLineup),
                ...
                ).setConstantHeadingInterpolation(wallPickup1.getHeading())
                .addParametricCallback(0, () ->setLiftArmSlidePreWallPickup())
                .addParametricCallback(0.5, () -> setWallSpecRemovalPosition())
                .build();
        ...
        return3 = follower.pathBuilder()
                ...
                ).setConstantHeadingInterpolation(wallPickup3.getHeading())
                .addParametricCallback(0.7, () ->setLiftArmSlidePreWallPickup())
                .addParametricCallback(0.8, () -> setWallSpecRemovalPosition())
                .build();
    ...
    private void pathUpdate() {
        switch (pathState) {
            ...
            case 1: // push specimen onto bar
                ...
                break;
            case 2: // retract arm before move
            ...
            case 3: // travel state and move three samples back
                if (SlideMotor.getCurrentPosition() < SlideMotor.getTargetPosition() + 15 && SlideMotor.getCurrentPosition() > SlideMotor.getTargetPosition() - 15) {
                    ...
                    follower.followPath(twoSamplePush, true);
                    pathState = 4;
                }
                break;
                //begin second sample
            ...
            case 17: // move to wall
                ...
                break;
                // begin sample 4
            case 18: //set timer for state 5
                if (!follower.isBusy()) {
                    timer.reset();
                    pathState = 19;
                }
                break;
            case 19: // grab from wall
                if (ChainLiftMotor.getCurrentPosition() < ChainLiftMotor.getTargetPosition()+5 && ChainLiftMotor.getCurrentPosition() > ChainLiftMotor.getTargetPosition()-3 && timer.milliseconds() > 700) {
                    gripperIntake();
                    setSlideForWallPickup();
                    pathState = 20;
                ...
                break;
            case 20: // pick specimen off wall
                if (SlideMotor.getCurrentPosition() > SlideMotor.getTargetPosition() - 10) {
                    rotateLiftArm(500, CHAIN_ARM_POWER);
                    pathState = 21;
                }
